...
<div class="bg-white rounded-lg shadow p-6 max-w-2xl">
    <form action="{{ route('landlord.properties.update', $property) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" value="{{ old('name', $property->name) }}" required class="mt-1 block w-full border border-gray-300 rounded p-2 @error('name') border-red-500 @enderror">
            @error('name') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
        </div>

        <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700">Address</label>
            <textarea name="address" required class="mt-1 block w-full border border-gray-300 rounded p-2 @error('address') border-red-500 @enderror">{{ old('address', $property->address) }}</textarea>
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <select name="type" required class="mt-1 block w-full border border-gray-300 rounded p-2 @error('type') border-red-500 @enderror">
                    ...
                    <option value="mixed" {{ old('type', $property->type) === 'mixed' ? 'selected' : '' }}>Mixed</option>
                </select>
            ...
            <div>
                ...
                <input type="number" name="rent_amount" step="0.01" value="{{ old('rent_amount', $property->rent_amount) }}" class="mt-1 block w-full border border-gray-300 rounded p-2 @error('rent_amount') border-red-500 @enderror">
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="number" name="bedrooms" value="{{ old('bedrooms', $property->bedrooms) }}" class="mt-1 block w-full border border-gray-300 rounded p-2 @error('bedrooms') border-red-500 @enderror">
                @error('bedrooms') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">Bathrooms</label>
                <input type="number" name="bathrooms" value="{{ old('bathrooms', $property->bathrooms) }}" class="mt-1 block w-full border border-gray-300 rounded p-2 @error('bathrooms') border-red-500 @enderror">
        ...
        <div class="mb-4">
            ...
            <textarea name="description" class="mt-1 block w-full border border-gray-300 rounded p-2 @error('description') border-red-500 @enderror">{{ old('description', $property->description) }}</textarea>
